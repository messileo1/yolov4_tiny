...
if __name__ == "__main__":
    ...
    #---------------------------------------------------------------------#
    #   fp16        是否使用混合精度训练
    #               可减少约一半的显存、但是需要pytorch1.7.1以上才能用。不过可以自己修改代码，用apex
    #---------------------------------------------------------------------#
    fp16            = False
    #---------------------------------------------------------------------#
    ...
    anchors_mask    = [[3, 4, 5], [1, 2, 3]] #一个grid cell有三个anchor，最后有两个有效特征层，第一个特征层用的是第456个anchor，第二个则用234
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    #   冻结阶段训练参数
    #   此时模型的主干被冻结了，特征提取网络不发生改变，只对neck和head进行训练
    #   占用的显存较小，仅对网络进行微调
    ...
    if distributed: # windows下只能设置distributed = False
    ...
    #   获取classes和anchor
    #----------------------------------------------------#
    class_names, num_classes = get_classes(classes_path) # 列表，列表长度
    anchors, num_anchors     = get_anchors(anchors_path) # 6行2列的矩阵，6
    ...
    if fp16:
        #------------------------------------------------------------------#
        #   torch 1.2不支持amp，建议使用torch 1.7.1及以上正确使用fp16。不过我安的是pytorch1.5.1，如果想用混合精度训练，建议采用apex，参考amp_test.py修改代码
    ...
    if True:
        ...
        #---------------------------------------#
        #   根据optimizer_type选择优化器，见YOLOv4的注释
        #---------------------------------------#
        pg0, pg1, pg2 = [], [], []  
        ...
        #   开始模型训练
        #---------------------------------------#
        for epoch in range(Init_Epoch, UnFreeze_Epoch): # 0，100
            #---------------------------------------#
            #   如果模型有冻结学习部分
            #   则解冻，并设置参数
            #---------------------------------------#
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train: # 解冻阶段，重新定义训练参数
                ...
                UnFreeze_flag = True # 冻结阶段结束，参数只设置一次就行了，所以if条件执行一次就行了。
